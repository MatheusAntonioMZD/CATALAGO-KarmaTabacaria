...
function criarMiniaturas(imagens, nome) {
    const container = document.querySelector('.produto-miniaturas');
    container.innerHTML = '';

    imagens.forEach((imagem, index) => {
        const miniatura = document.createElement('div');
        miniatura.className = `produto-miniatura ${index === 0 ? 'ativa' : ''}`;
        miniatura.innerHTML = `<img src="${imagem}" alt="${nome} - Imagem ${index + 1}">`;
        
        miniatura.addEventListener('click', () => {
            // Atualiza a imagem principal
            document.getElementById('produto-imagem-principal').src = imagem;
            // Atualiza a classe ativa
            document.querySelectorAll('.produto-miniatura').forEach(m => m.classList.remove('ativa'));
...
function carregarDetalhesProduto() {
    ...
    console.log('ID do produto:', produtoId); // Debug
    
    // Procura o produto nos dois arrays
    let produto = produtos.find(p => p.id === parseInt(produtoId));
    
    // Se não encontrou nos produtos normais, procura no array de piteiras
    if (!produto) {
        const piteira = piteiras.find(p => p.id === parseInt(produtoId));
        if (piteira) {
            produto = piteira;
        }
    }
    ...
    if (produto) {
        ...
        // Atualiza a imagem principal com tratamento de erro
        ...
        imagemPrincipal.onerror = function() {
            ...
            this.src = 'caminho/para/imagem/padrao.jpg'; // Substitua por uma imagem padrão
        };
        ...
        const especificacoesLista = document.getElementById('produto-especificacoes-lista');
        especificacoesLista.innerHTML = produto.especificacoes.map(spec => `<li>${spec}</li>`).join('');
        
        // Cria as miniaturas
        criarMiniaturas(produto.imagens, produto.nome);
    } else {
        console.error('Produto não encontrado para o ID:', produtoId); // Debug
        window.location.href = 'index.html';
    }
}

// Carregar os detalhes quando a página carregar
